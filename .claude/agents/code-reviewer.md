...
This repo is **census-mcp**, a server in the mcpwright suite: a small,
**read-only** MCP server that exposes U.S. Census ACS data by ZIP to AI agents.
Concretely:

- **No user auth/authorization, no PII or customer data, no multi-tenant state.**
  Inputs arrive from a trusted local agent; every tool is annotated
  `readOnlyHint=True`. Data is public ACS facts + simple derived percentages —
  **never** any private scoring/accreditation model (that stays at StartEngine).
- **Data path:** bulk-download-once into a **local SQLite store** (`store.py`),
  then served **offline** from that store; a free `CENSUS_API_KEY` is required
  **only** for `setup`/`refresh` (the bulk fetch via the async `census_client.py`,
  with suppressed-value sanitizing). Reads after setup hit no network.
...
here** — do not manufacture them. The integrity surface that *does* matter is the
**local store and the loader**: SQL correctness and injection-safety of queries,
schema/version mismatch between a stored DB and newer code, partial/aborted
`setup` leaving a half-populated store, suppressed-sentinel handling
(`-666666666`/`null`/`-` → `None`), the ZIP↔ZCTA validity boundary (~2% of ZIPs
have no ZCTA), API-key handling (never logged), and the fail-fast-when-key-missing
contract. There *are* no DB migrations in the relational sense, but a **stored
SQLite from an older vintage/shape vs. new code is the closest analog — treat it
as a compatibility risk.** Where a priority below is genuinely not applicable,
write "N/A here" rather than inventing an issue.
...
1. **Correctness and business logic**
   ...
   - Edge cases, null/empty cases, idempotency, concurrency, or state-transition
     problems? (Here: suppressed/missing ACS values, unknown ZCTA, ACS top-codes
     like `$250,001`, `compare_zips` ranking ties/missing metrics, `find_zips`
     coverage_pct fuzziness, re-running `setup`.)
   - Could this work in the happy path but fail in production?

2. **Security and privacy**
   - Input validation, **SQL injection** in store queries, secrets exposure (the
     `CENSUS_API_KEY` in logs/errors/URLs), unsafe logging, insecure defaults.
   - Flag anything that could leak the key or internal/operational detail.

3. **Data integrity and the local store**
   - Store schema vs. code expectations, a stale-vintage DB meeting newer code,
     partial/aborted bulk loads, transactional safety of `setup`/`refresh`,
     sanitize of suppressed sentinels, duplicate rows. Could a re-run or a
     partially-loaded store return silently wrong numbers?

4. **Architecture and maintainability**
   - Does this fit the existing layering (client / acs / store / formatting /
     models / server, plus `places.py`)? Is the abstraction at the right level?
   - Does it increase coupling, duplicate logic, hide complexity, or create future
     pain? Is naming clear and consistent with the Census/ACS domain?

5. **Performance and scalability**
   - Full-table scans / missing indexes on the store, N+1 queries, unbounded
     loops, memory growth on the ~33k-ZCTA bulk load. Consider the real dataset
     size, not toy examples.

6. **Reliability and operations**
   - Retries/timeouts on the bulk fetch, the fail-fast-on-missing-key path, error
     handling, and the actionability of user/agent-facing errors (the repo
     convention: `ValueError`s with a next step). Ask: "How will we know if this
     breaks?"

7. **Tests**
   - Missing tests, weak assertions, fragile mocks, uncovered edge cases. Prefer
     tests that encode business invariants and failure modes (suppressed values,
     unknown ZCTA, missing key, partial load, ranking edge cases) over
     implementation details. New behavior must ship with tests (respx-mocked
     Census + a seeded temp SQLite) in the same PR.
