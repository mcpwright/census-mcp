#!/usr/bin/env bash
# Build the census-mcp MCPB desktop-extension bundle.
#
# Produces dist/mcpwright-census-<version>.mcpb — a `uv`-type bundle: it ships
...
out="$root/dist/mcpwright-census-$version.mcpb"

stage="$(mktemp -d)/census"
mkdir -p "$stage"
